-- =====================================================================
-- Reference prices: an admin-curated table of approximate baseline
-- prices for common products (e.g. "نوشابه قوطی 330cc کوکاکولا").
--
-- This is deliberately NOT auto-populated from any external source —
-- nobody (including me) has a reliable feed of live Iranian wholesale
-- prices, and guessing numbers would be actively misleading. Instead,
-- an admin who knows the market enters a number they trust, and the
-- site treats it as a labeled estimate, not a live quote. It becomes
-- the default/baseline shown across the site (market comparison page,
-- and as a price suggestion when a supplier adds a matching product)
-- until real transaction data accumulates.
-- =====================================================================
