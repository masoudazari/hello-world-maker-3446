-- =====================================================================
-- User-reported prices: instead of one admin manually tracking prices
-- for a large and constantly-changing catalog (which doesn't scale),
-- any signed-in user can report a price they personally observed
-- somewhere. Each report is a single real data point, clearly
-- attributed to "user-reported" (not scraped, not guessed), and the
-- catalog's price coverage grows organically with platform usage
-- instead of depending on one person's manual effort.
--
-- Validation happens inside a SECURITY DEFINER function rather than a
-- permissive RLS insert policy, so a client can never forge a
-- different source, backdate fetched_at, or write arbitrary columns.
-- =====================================================================
