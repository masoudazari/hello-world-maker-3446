...
-- Builds on existing: purchase_requests, supplier_offers, suppliers,
-- products, notifications. Additive only.
--
-- What this adds:
--  1) offer_versions       — full negotiation history per offer
--  2) request_conversations / request_messages
--                           — a chat isolated per (request, supplier),
--                             completely separate from the general
--                             /messages inbox (which stays order-based)
--  3) request_recipients    — smart fan-out with an anti-spam cap
-- Existing RLS on supplier_offers already prevents a supplier from
-- reading another supplier's offer on the same request (checked: the
-- "offers parties read" policy scopes to `supplier_id = my_supplier_id()`
-- OR the buyer), so offer-price privacy between sellers is already
-- enforced at the database level — this migration does not need to
-- touch that policy.
-- =====================================================================
...
-- Auto-create the isolated conversation the moment a supplier submits an offer,
-- and notify the buyer. This is completely separate from public.messages.
...
-- Notify the other party whenever a request_message is sent.
...
-- On a new request, find suppliers with an active product in the same
-- category, rank them by supplier_score, cap at max_recipients, and
-- notify only those (this is the "20 عمده‌فروش مرتبط" behaviour).
